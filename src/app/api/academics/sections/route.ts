...
import { EnrollmentStatus, Gender, Role } from "@/prisma/generated";
...
export async function GET(_: Request) {
  try {
    ...
    const result = sections.map((section) => {
      ...
      const femaleCount =
        counts.find((g) => g.gender === "FEMALE")?.count ?? 0n;
    ...
    return new Response(
      JSON.stringify(
        { sections: result, totalSections, totalStudents, totalAvailableSeats },
...
export async function POST(req: Request) {
  try {
    ...
    return new Response(
      JSON.stringify(newSection, (_, value) =>
